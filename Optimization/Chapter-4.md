...
```ad-note
Για επαναληπτική μέθοδο που κατασκευάζει ακολουθία $\vec{x_k}$ που συγκλίνει στη λύση $\vec{x^*}$, καθώς $\lim_{k\to\infty}$ μπορούμε να ορίσουμε τον ρυθμό σύγκλισης.

Πρώτα έχουμε την ακολουθία των λαθών $$\vec{e_k} = \vec{x_k} - \vec{x^*}$$

για την οποία έχουμε $\lim_{k\to\infty}\vec{e_k} = \vec{0}$.

Η ακολουθία $\vec{x_k}$ συγκλίνει στο $\vec{x^*}$ με ρυθμό $r$ και σταθερά του ρυθμού $c$ αν

$$\lim_{k\to\infty}\frac{||\vec{e_{k+1}}||}{||\vec{e_k}||} = c$$

με $c < \infty$

Μπορούμε να υποθέσουμε, για να μην πρέπει να αντιμετωπίσουμε το όριο, ότι έχουμε

$$||\vec{e_{k+1}}|| = c ||\vec{e_k}||^r$$

Διακρίνουμε τις εξής περιπτώσεις:

1. Αν $r = 1$, τότε $||\vec{e_{k+1}}|| = c ||\vec{e_k}||$, και η σύγκλιση ονομάζεται γραμμική (linear)
2. Αν $r \ge 1$ και $c = 0$ τότε η σύγκλιση ονομάζεται superlinear (υπεργραμμική)
3. Αν $ r= 2$ τότε η σύγκλιση είναι τετραγωνική (quadratic)
4. Αν $1 \le r \le 2$ τότε η σύγκλιση είναι superlinear (εμφανίζεται σε προβλήματα βελτιστοποίησης που συμμετέχουν μόνο πρώτες παράγωγοι)
```
